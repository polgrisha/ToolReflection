...
def call_api_function(input_params, openapi_spec, path, method, base_url=None):
    ...
    for param_doc in function_doc.get("parameters", []):
        ...
        if param_doc["name"] in input_params:
            ...
            type_check_error.extend(type_check(param_doc["name"], param_doc['schema'], input_params))
    ...
    if required_params or required_body_params:
        ...
        if required_body_params:
            ...
            error_str = f"Missing required parameters: {', '.join(required_body_params)}. You need to change the input and try again."
            error_str = "Status Code: 400. Response: {\"message\": \"" + error_str + "\""
            error_str += ". You should choose one of: (1) change the input and retry; (2) return the 'Final Answer' and explain what happened; (You must choose this one when the error occurs more than 3 times.) (3) call another function."
            raise ValueError(error_str)
        else:
            error_str = f"Missing required parameters: {missing_params}. You need to change the input and try again."
            error_str = "Status Code: 400. Response: {\"message\": \"" + error_str + "\""
            error_str += ". You should choose one of: (1) change the input and retry; (2) return the 'Final Answer' and explain what happened; (You must choose this one when the error occurs more than 3 times.) (3) call another function."
            raise ValueError(error_str)
