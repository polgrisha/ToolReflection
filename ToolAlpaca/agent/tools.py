    ...
    def __init__(self, base_url, func_name, openapi_spec, path, method, description, retrieval_available=False, **kwargs):
        ...
        def func(params):
            try:
                ...
                message = "Invalid JSON format. Please ensure 'Action Input' is a valid JSON object."
                message = "Status Code: 400. Response: {\"message\": \"" + message + "\""
                message += ". You should choose one of: (1) change the input and retry; (2) return the 'Final Answer' and explain what happened; (You must choose this one when the error occurs more than 3 times.) (3) call another function."
                return message
            retry_times = 0
...
class CustomInvalidTool(BaseTool):
    ...
    def _run(self, tool_name: str, all_tools: List[str]) -> str:
        """Use the tool."""
        message = f"`{tool_name}` is not a valid action. The action must be one of {all_tools}."
        message = "Status Code: 400. Response: {\"message\": \"" + message + "\""
        message += ". You should choose one of: (1) change the input and retry; (2) return the 'Final Answer' and explain what happened; (You must choose this one when the error occurs more than 3 times.) (3) call another function."
        return message
    ...
    def __init__(self, **kwargs):
        def func(params):
            ...
            if "Question" not in question:
                message = 'For `getDetails`, the "Action Input" must be like {Question": "The question to prompt user to provide sufficient information."}.'
                message = "Status Code: 400. Response: {\"message\": \"" + message + "\""
                message += ". You should choose one of: (1) change the input and retry; (2) return the 'Final Answer' and explain what happened; (You must choose this one when the error occurs more than 3 times.) (3) call another function."
                return message
