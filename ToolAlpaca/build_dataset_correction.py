...
def build_dataset(api_info):
    ...
    for ans in api_info.get("Instances", []):
        ...
        used_tools = set()
        for idx, step in enumerate(ans["intermediate_steps"]):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prev_data_path", type=str, default="")
    ...
    if args.prev_data_path:
        prev_train = json.load(open(args.prev_data_path, 'r'))
        all_data = prev_train + all_data
        random.shuffle(all_data)
    print(len(prev_train))
    print(len(all_data))
