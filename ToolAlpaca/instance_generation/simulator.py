...
from utils import async_openai_chat_completions, parse_json_string, openai_chat_completions
...
def generate_request(method, api_name, request_url, headers, data):
    ...
    data_str = f"-d '{json.dumps(str(data))}'" if data else ""
...
async def _get_response(prompt):
    ...
    output = openai_chat_completions(prompt, temperature=0.5)
    usage = output["usage"]["total_tokens"]
    output = output["choices"][0]["message"]["content"]
    ##################### mocked response
    # usage = 100
    # output = open('prompts/Sample_gpt_response.txt', 'r').read()
    ##################### mocked response
